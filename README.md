...
Приложение для расшифровки файлов обмена DataMobile в форматах
`.dm` и `.dmU`.
...
3. Перетащите файл `.dm` или `.dmU` в окно приложения либо нажмите
   **Выбрать файл…** и укажите его.
4. Расшифровка выполнится сразу после выбора файла — нажимать отдельную
   кнопку не нужно. При смене формата файл расшифровывается заново.

Результат выводится таблицей: поля сгруппированы по разделам, расшифровка
служебных значений выделена цветом, пустые поля показаны приглушённо.

- **Скрыть пустые поля** — оставить в результате только переданные значения;
- **Копировать** — скопировать результат в буфер обмена как текст;
- **Обновить** (`F5`, в macOS `⌘R`) — перечитать файл, если он изменился на диске.

Оформление автоматически подстраивается под светлую или тёмную тему системы.
...
Версия приложения хранится в одном месте — в файле `VERSION`. Из него её
берут окно программы, сборки macOS (`Info.plist`) и Windows (свойства `.exe`),
а также установщик Inno Setup.
...
├── result_view.py                  оформление результата
├── theme.py                        цвета и стили интерфейса
